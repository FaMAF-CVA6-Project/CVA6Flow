...
rather than converted into a JSON whose instruction column would be blank.
    ...
    python3 scripts/create_all_CVA6Flow_jsons.py --force    # redo existing JSONs
"""
import argparse
import os
import subprocess
...
# This script belongs to the CVA6Flow repository and works inside it only.
...
def main():
    ...
    if not os.path.isfile(TRACER):
        print(f"[ERROR] No {TRACER}. This script runs inside the "
              f"CVA6Flow repository, beside its tracer.")
        return 2
