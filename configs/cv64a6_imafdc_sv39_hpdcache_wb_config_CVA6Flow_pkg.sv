...
//              subsystem, with the table of cuts CVA6Flow was developed on
//
// ---------------------------------------------------------------------------
// NOTICE OF MODIFICATION
//
// This file has been modified by the FaMAF CVA6 Project, Universidad Nacional
// de Cordoba, and is NOT the upstream file. The unmodified original is beside
// it as cv64a6_imafdc_sv39_hpdcache_wb_config_pkg.sv, and in the CVA6 tree at
// core/include/cv64a6_imafdc_sv39_hpdcache_wb_config_pkg.sv.
//
// What changed: a table of the seventeen configuration cuts used while
// developing the CVA6Flow viewer was added, with one selector,
// CVA6_CONFIG_SEL. Every parameter a cut varies now reads the selected value
// through an Sw* localparam, and four localparams were added to name values
// the original wrote inline: CVA6ConfigNrCommitPorts,
// CVA6ConfigMaxOutstandingStores, CVA6ConfigInstrTlbEntries and
// CVA6ConfigDataTlbEntries. CFG_BASELINE elaborates the original core.
//
...
package cva6_config_pkg;
  ...
  // Available configurations, one per row: name, id, parameter cut and workload
  ...
  localparam int CFG_BTB_4         = 7;   // BTBEntries 32 -> 4               : direct_call_return_test
  ...
  localparam int CFG_DTLB_1        = 11;  // DataTlbEntries 16 -> 1           : daxpy (M-mode, no TLB)
  localparam int CFG_MAXOS_1       = 12;  // MaxOutstandingStores 7 -> 1      : store_miss_test
  localparam int CFG_ICACHE_4K     = 13;  // IcacheByteSize 16K -> 4K         : icache_footprint_test
  ...
  localparam int CFG_MIX           = 17;  // every swept knob at its tightest : daxpy

  // Four cuts leave every counter of their workload unchanged, measured on
  // 2026-09-14, so their rows show the baseline under another name:
  //   CFG_BTB_4     the BTB serves only non-return jalr, and the workload has none
  //   CFG_DTLB_1    daxpy runs in M-mode, which translates no address
  //   CFG_MAXOS_1   read only by the std and write-through caches, not the HPDcache
  //   CFG_COMMIT_1  the second commit port never shortens commit_ilp_test's region
  ...
  localparam int SwDcacheByteSize =
      (CVA6_CONFIG_SEL == CFG_DCACHE_16K || CVA6_CONFIG_SEL == CFG_MIX) ?
          16384 : CVA6ConfigDcacheByteSize;
  localparam int SwDcacheSetAssoc =
      (CVA6_CONFIG_SEL == CFG_DCACHE_ASSOC2 || CVA6_CONFIG_SEL == CFG_MIX) ?
          2 : CVA6ConfigDcacheSetAssoc;
  ...
  localparam int SwBTBEntries =
      (CVA6_CONFIG_SEL == CFG_BTB_4 || CVA6_CONFIG_SEL == CFG_MIX) ?
          4 : CVA6ConfigBTBEntries;
  ...
  localparam int SwNrLoadBufEntries =
      (CVA6_CONFIG_SEL == CFG_LOADBUF_1 || CVA6_CONFIG_SEL == CFG_MIX) ?
          1 : CVA6ConfigNrLoadBufEntries;
  localparam int SwDataTlbEntries =
      (CVA6_CONFIG_SEL == CFG_DTLB_1 || CVA6_CONFIG_SEL == CFG_MIX) ?
          1 : CVA6ConfigDataTlbEntries;
  localparam int SwMaxOutstandingStores =
      (CVA6_CONFIG_SEL == CFG_MAXOS_1 || CVA6_CONFIG_SEL == CFG_MIX) ?
          1 : CVA6ConfigMaxOutstandingStores;
  localparam int SwIcacheByteSize =
      (CVA6_CONFIG_SEL == CFG_ICACHE_4K || CVA6_CONFIG_SEL == CFG_MIX) ?
          4096 : CVA6ConfigIcacheByteSize;
  localparam int SwIcacheSetAssoc =
      (CVA6_CONFIG_SEL == CFG_ICACHE_DM || CVA6_CONFIG_SEL == CFG_MIX) ?
          1 : CVA6ConfigIcacheSetAssoc;
  localparam int SwNrCommitPorts =
      (CVA6_CONFIG_SEL == CFG_COMMIT_1 || CVA6_CONFIG_SEL == CFG_MIX) ?
          1 : CVA6ConfigNrCommitPorts;
  localparam int SwRASDepth =
      (CVA6_CONFIG_SEL == CFG_RAS_0 || CVA6_CONFIG_SEL == CFG_MIX) ?
          0 : CVA6ConfigRASDepth;
