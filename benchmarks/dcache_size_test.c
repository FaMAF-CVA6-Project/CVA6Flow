/* Paired with the DcacheByteSize 32K -> 16K cut: three read-modify-write
 * passes over 6144 ints, 24 KiB of data, more than the halved D-cache holds,
 * so the later passes miss lines the full one kept. */
...
#define ARR_SIZE 6144
#define REPEATS 3

static volatile int buffer[ARR_SIZE];
