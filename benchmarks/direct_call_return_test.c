/* 24 direct calls per iteration from distinct sites to four small functions,
 * and their returns. The front end resolves each jal itself and the RAS
 * predicts each ret, so the BTB, which serves only other jalr, sits idle. */
...
__attribute__((noinline)) static int g0(int x) { return x * 3 + 1; }
__attribute__((noinline)) static int g1(int x) { return x ^ 0x55; }
__attribute__((noinline)) static int g2(int x) { return x + 7; }
__attribute__((noinline)) static int g3(int x) { return (x >> 1) | (x << 3); }
