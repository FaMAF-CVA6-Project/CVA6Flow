/* Paired with the BHTEntries cuts: 96 conditional branches, each at its own
 * PC and always going the way a fixed pseudo-random pattern set, so a smaller
 * table makes branches that go opposite ways share a counter. */
