/* Paired with the MaxOutstandingStores 7 -> 1 cut: two passes that write
 * every one of 16,384 ints, a store per iteration, so a store buffer holding
 * one outstanding store makes the loop wait on each write. */
