...
`scripts/create_all_CVA6Flow_jsons.py` converts a whole folder at once, skipping any VCD with no listing beside it and any JSON already newer than its VCD, and lets the tracer's progress line through so a long conversion does not look hung. It passes `--strict` to the tracer by default, so a degraded trace ends the batch with exit 3 while its JSON is still written, and `--no-strict` turns that off.
